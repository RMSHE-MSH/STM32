#ifndef __EXIT_IT_H__
#define __EXIT_IT_H__
...
*/
#define EXIT_IRQHandler EXTI1_IRQHandler    //中断函数(中断函数的名字是由STM32汇编启动程序指定的,不可更改)


//初始化连接LED的Pin口(GPIOx, GPIO_Pin, EXTI_Trigger = 默认下降沿触发中断, NVIC_PriorityGroup = 默认为2位抢占2位响应);
void EXIT_it_init(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, EXTITrigger_TypeDef EXTI_Trigger, uint32_t _NVIC_PriorityGroup);
